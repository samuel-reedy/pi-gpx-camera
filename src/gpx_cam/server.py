...
# camera = PiCamera2(sensor_mode=2, resolution='1920x1080', framerate=15)
# # camera = PiCamera(sensor_mode=2, resolution='1640x1232', framerate=30)
...
# picam2.switch_mode_and_capture_file(capture_config, "picamer2_test_full.jpg")

# # # # We should be able to set the lens position and see it reported back.
# # # picam2.set_controls({'LensPosition': 10, 'FrameRate': 30})
# # # time.sleep(0.5)
...
# # picam2.switch_mode_and_capture_file(capture_config, "picamer2_test_full_focus_1.5.jpg")
# # time.sleep(0.1)
# picam2.stop()
...
picam2.start()
# picam2.set_controls({'LensPosition': 10, 'FrameRate': framerate})

# # video_config = picam2.create_video_configuration({"size": (2028, 1520)})
# # video_config = picam2.create_video_configuration({"size": (2028, 1120)})
    ...
    def post(self):
        ...
        if is_recording:
            ...
            def record():
                ...
                print(f'Recording  ../../data/{record_filename}.avi')
                count = 20
                ...
                output = FfmpegOutput(f'../../data/{record_filename}.avi',)
                output.start()
                for i in range (20):
                    if DO_RECORD == False:
                        break
...
def keyboard_handler(fd, events):
    ...
    if key == b't':
        ...
        for i in range(10):
            ...
            arr = picam2.capture_array()
            # arr = request.make_buffer('main')
            # reshape = arr.reshape((arr.shape[0], arr.shape[1]//2, 2))
            # arr = request.make_image('main')
            shp = arr.shape
            # arr = arr[:shp[0]//2, :shp[1]//2, :]
            # arr = np.ascontiguousarray(arr)
            print(f"array {arr.shape = }")
            # _ = qoi.write(f"img-{file_cnt}.qoi", arr)
            # # filename = f"/home/pi/usb/rpi-test-{main_stream['size']}-{i}.jpg"
            # filename = f"rpi-{CAM_TYPE}-{main_stream['size']}-{file_cnt}.jpg"
            file_cnt += 1
...
def process_mavlink_data():
    ...
    while True:
        ...
        try: 
            ...
            if msg is not None:
                pass
            #     mavlink_queue.put(msg)

            #     # timestamp = the_connection.time_since('GLOBAL_POSITION_INT')
            #     # print(f"Received message {msg}")
            #     # print("Altitude is ", altitude_str)
